...
7. Every exported server function is a public endpoint for anyone who can open the web app, and runs as the deployer (`executeAs: USER_DEPLOYING`) or as the visitor (`USER_ACCESSING`, the "people the sheet is shared with" mode). Functions that **write** must call `assertDeployer()` (see `seedSampleData`; it checks deployer and spreadsheet owner so it works in both modes) or otherwise authorize; validate arguments. `webapp.access` defaults to `MYSELF` — widen only deliberately to `DOMAIN` (whole Workspace domain) or `USER_ACCESSING` + `ANYONE` (sheet-shared users) (docs/deploy.md §7).
